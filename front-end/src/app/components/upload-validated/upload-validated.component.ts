...
@Component({
  ...
  templateUrl: './upload-validated.component.html',
  styleUrl: './upload-validated.component.scss'
})

  export class UploadValidatedComponent implements OnInit {
    ...
    ngOnInit(): void {
      ...
      if (this.userId) {
        this.loadValidatedDocuments(this.userId); // Pass user ID to load documents
      }
    }
  
    loadValidatedDocuments(userId: number): void {
      ...
      if (confirm('Are you sure you want to delete this document?')) {
        this.documentService.deleteDocument(documentId).subscribe({
          next: () => {
            ...
            this.loadValidatedDocuments(this.userId!); // Reload documents after deletion
          },
          error: (error) => {
